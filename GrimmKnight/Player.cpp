...
bool Player::TakeDamage(uint damage, Direction dir)
{
    ...
    GrimmKnight::audio->Play(PLAYER_HURT);
    ...
    if (state == STILL || state == WALKING || state == JUMPING || state == FALLING)
    {
        ...
        if (window->KeyDown('Z'))
        {
            if (jumpKeyCtrl && state != FALLING)
                GrimmKnight::audio->Play(PLAYER_JUMP);
        ...
        if (window->KeyDown('X') && attackCd.Up() && attackKeyCtrl)
        {
            ...
            GrimmKnight::audio->Play(PLAYER_ATTACK);
            ...
            GrimmKnight::scene->Add(atk, MOVING);
        }
        if (window->KeyUp('X'))
            attackKeyCtrl = true;
        ...
        if (fireball)
        {
            if (window->KeyDown('S') && fireballCd.Up() && fireballKeyCtrl && HasMana())
            {
                ...
                GrimmKnight::scene->Add(fb, MOVING);
            }
            if (window->KeyUp('S'))
        ...
        if (dash)
        {
            if (window->KeyDown('C') && dashCd.Up() && dashKeyCtrl && dashGroundCtrl)
            {
                ...
                GrimmKnight::audio->Play(PLAYER_DASH);
...
update : {
    ...
    case WALKING:
        if (walkingCtrl)
            GrimmKnight::audio->Play(PLAYER_WALK);
        walkingCtrl = false;
        break;
    default:
        GrimmKnight::audio->Stop(PLAYER_WALK);
        walkingCtrl = true;
        break;
    }
...
void Player::Draw()
{
    if (GrimmKnight::currentLevel != LEVELFINAL)
...
void Player::OnCollision(Object *other)
{
    ...
    case WALL_TOP: {
        ...
        if (justEntered && isInside)
        {
            ...
            {
                landingCtrl = false;
                GrimmKnight::audio->Play(PLAYER_LAND);
            }
    ...
    case ENTITY_BLOCK_BOSS_ACTIVATOR: {
        ...
        GrimmKnight::scene->Delete(activator, STATIC);
    }
    }
}
