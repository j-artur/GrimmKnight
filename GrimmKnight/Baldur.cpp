...
Baldur::Baldur(int iX, int iY)
{
    ...
    GrimmKnight::scene->Add(rightWall, STATIC);
    topWall = new TopWall(iX - 2, iY - 2, 4);
    GrimmKnight::scene->Add(topWall, STATIC);
}
...
void Baldur::Update()
{
    ...
    case BALDUR_IDLE:
        ...
        {
            if (soundCtrl)
                GrimmKnight::audio->Play(BALDUR_BLOCK_SOUND);
            state = BALDUR_BLOCK;
...
void Baldur::OnCollision(Object *other)
{
    ...
    {
        GrimmKnight::baldurKilled = true;
        hp = 0;
        hurtCd.Restart();
        state = BALDUR_DEAD;
        GrimmKnight::scene->Delete(actionArea, MOVING);
        GrimmKnight::scene->Delete(rightWall, STATIC);
        GrimmKnight::scene->Delete(topWall, STATIC);
        deathCd.Restart();
    }
}
