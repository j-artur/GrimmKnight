...
Level *GrimmKnight::level = nullptr;
Player *GrimmKnight::player = nullptr;
Audio *GrimmKnight::audio = nullptr;
Scene *GrimmKnight::scene = nullptr;
Cursor *GrimmKnight::cursor = nullptr;
Cooldown GrimmKnight::levelTransition{2.0f};
Cooldown GrimmKnight::fireballCd{4.0f};
Cooldown GrimmKnight::dashCd{4.0f};
LevelId GrimmKnight::currentLevel = TITLESCREEN;
Scene *GrimmKnight::pausedScene = nullptr;
bool GrimmKnight::viewBBox = false;
bool GrimmKnight::paused = false;
bool GrimmKnight::baldurKilled = false;
bool GrimmKnight::fkDefeated = false;
bool GrimmKnight::transitioning = false;
bool GrimmKnight::gettingFireball = false;
bool GrimmKnight::gettingDash = false;
bool GrimmKnight::playerDead = false;

void GrimmKnight::Init()
{
...
void GrimmKnight::StartTransition()
{
    transitioning = true;
    levelTransition.Restart();
}

void GrimmKnight::GetFireball()
{
    gettingFireball = true;
    fireballCd.Restart();
}

void GrimmKnight::GetDash()
{
    gettingDash = true;
    dashCd.Restart();
}
                     ...
                     _In_ int nCmdShow)
{
    ...
    engine->window->Title("GrimmKnight");
    engine->window->Icon(IDI_ICON);
    engine->window->Cursor(IDC_CURSOR);
    engine->window->HideCursor(true);

    int status = engine->Start(new GrimmKnight());
