...
void Level3::Init()
{
    ...
    GrimmKnight::audio->Play(LEVEL3_MUSIC, true);
    GrimmKnight::audio->Volume(LEVEL3_MUSIC, 0.5f);
...
{
    GrimmKnight::audio->Stop(LEVEL3_MUSIC);
    delete background;
    delete foreground;
    delete totemLeft;
    delete totemRight;
...
void Level3::EnterFrom(LevelId id)
{
    ...
    {
    case LEVEL1:
        GrimmKnight::player->MoveTo(2560.0f, 1442.0f);
    ...
    case GAMEOVERSCREEN:
    default:
        GrimmKnight::player->MoveTo(2448.0f, 1442.0f);
        scene->Apply([&](Object *obj) { obj->Translate(float(-window->Width()), float(-window->Height())); });
        GrimmKnight::player->Dir(H_LEFT);
        break;
    }
