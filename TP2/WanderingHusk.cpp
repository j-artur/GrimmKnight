...
WanderingHusk::WanderingHusk(TileSet* tileSet, int iX, int iY)
{
    ...
    animation->Add(WH_WALKING * RIGHT, seqWalkRight, 2);
    animation->Add(WH_WALKING * LEFT, seqWalkLeft, 2);
    animation->Add(WH_PRE_RUNNING * RIGHT, seqPrepRight, 2);
    animation->Add(WH_PRE_RUNNING * LEFT, seqPrepLeft, 2);
    animation->Add(WH_RUNNING * RIGHT, seqRunRight, 2);
    animation->Add(WH_RUNNING * LEFT, seqRunLeft, 2);
    animation->Add(WH_HURTING * RIGHT, seqHurtRight, 1);
    animation->Add(WH_HURTING * LEFT, seqHurtLeft, 1);
    animation->Add(WH_DEAD * RIGHT, seqDeadRight, 1);
    animation->Add(WH_DEAD * LEFT, seqDeadLeft, 1);

    animation->Select(WH_WALKING * RIGHT);
...
bool WanderingHusk::TakeDamage(uint damage, AttackDirection atkDir)
{
...
void WanderingHusk::Update()
{
    ...
    case WH_RUNNING:
        if (runCd.Down())
        {
            xSpeed = direction == LEFT ? -runningSpeed : runningSpeed;
        }
        else
            state = WH_WALKING;
        break;
